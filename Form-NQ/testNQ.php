...
if ($_SERVER["REQUEST_METHOD"] === "POST") 
{
 ...
 $atr3entryval2 = $atr3entryval3 = 0;
  ...
  $atr3selectrisk = $_POST["atr3selectrisk"];
    ...
    $retestprice = round(($texbox2 + $tval3)*4)/4;
    $atr1shortentryprice = round(($texbox2 + $atr1entryval1)*4)/4;
    $atr1shortstopprice = round(($texbox2 + $atr1entryval2)*4)/4;
    ...
    $rightatr3stoporderprice = round(($texbox3 - $atr3entryval2) * 4) / 4;
    ...
    if ($selectrisk == "riskpoint") {
        ...
        $riskpoints = ($entprice - $stopprice) *4;
    }
    ...
    if ($riskpoints != 0) { // Check for division by zero
        if ($selectrisk == "riskpoint") {
            $nqcontracts = ($riskval / $riskpoints) / 20;
        } elseif ($selectrisk == "riskticks") {
            $nqcontracts = (($riskval / $riskpoints) / 20) *4;
        }
    }
    ...
    if ($atr1selectrisk == "riskpoint") {
        ...
        if ($atr1riskpoints != 0) {
            $atr1escontracts = ($riskval2 / $atr1riskpoints) / 20;
        ...
        $atr1riskpoints = ($atr1shortstopprice - $atr1shortentryprice)*4;
        if ($atr1riskpoints != 0) {
            $atr1escontracts = (($riskval2 / $atr1riskpoints) / 20 )* 4;
    ...
    if ($atr2selectrisk == "riskpoint") {
        ...
        if ($atr2riskpoints != 0) {
            $atr2escontracts = ($riskval2 / $atr2riskpoints) / 20;
            $atr2riskreward = ($atr2shortentryprice - $atr2shortexitprice) / $atr2riskpoints;
        }
    } elseif ($atr2selectrisk == "riskticks") {
        $atr2riskpoints = ($atr2shortstopprice - $atr2shortentryprice)*4;
        if ($atr2riskpoints != 0) {
            $atr2escontracts = (($riskval2 / $atr2riskpoints) / 20 )* 4;
    ...
    if ($atr3selectrisk == "riskpoint") {
        ...
        if ($atr3riskpoints != 0) {
            $atr3escontracts = ($riskval2 / $atr3riskpoints) / 20;
            $atr3riskreward = ($atr3shortentryprice - $atr3shortexitprice) / $atr3riskpoints;
        }
    } elseif ($atr3selectrisk == "riskticks") {
        $atr3riskpoints = ($atr3shortstopprice - $atr3shortentryprice)*4;
        if ($atr3riskpoints != 0) {
            $atr3escontracts = (($riskval2 / $atr3riskpoints) / 20 )* 4;
