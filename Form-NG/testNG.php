...
if ($_SERVER["REQUEST_METHOD"] === "POST") 
{
 ...
 $atr3entryval2 = $atr3entryval3 = 0;
  ...
  $atr3selectrisk = $_POST["atr3selectrisk"];
    ...
    //Textbox2
    $entprice = round(($texbox2 + $entval) * 1000) / 1000;
    $validaprice = round(($texbox2 + $texbox1) * 1000) / 1000;
    $retestprice = round(($texbox2 + $tval3) * 1000) / 1000;
    $atr1shortentryprice = round(($texbox2 + $atr1entryval1) * 1000) / 1000;
    // $atr1shortstopprice = intval(($texbox2 + $atr1entryval2) * 1000) / 1000;
     if ($selectmain == "ATRActual") {
        $atr1shortstopprice = intval(($texbox2 + $atr1entryval2)*1000)/1000;
        
    } elseif ($selectmain == "ATRTicks") {
        $atr1shortstopprice = round(($texbox2 + $atr1entryval2)*1000)/1000;
    } else {
        $atr1shortstopprice=0;
    }
    
    $atr1shortexitprice = round(($texbox2 + $atr1entryval3) * 1000) / 1000;

    $atr2shortentryprice = round(($texbox2 + $atr2entryval1) * 1000) / 1000;
    $atr2shortstopprice = round(($texbox2 + $atr2entryval2) * 1000) / 1000;
    $atr2shortexitprice = round(($texbox2 + $atr2entryval3) * 1000) / 1000;

    $atr3shortentryprice = round(($texbox2 + $atr3entryval1) * 1000) / 1000;
    $atr3shortstopprice = round(($texbox2 + $atr3entryval2) * 1000) / 1000;
    $atr3shortexitprice = round(($texbox2 + $atr3entryval3) * 1000) / 1000;
    ...
    //TextBox3
    $stopprice = round(($texbox3 - $entval) * 1000) / 1000;
    $invalidprice = round(($texbox3 - $texbox1) * 1000) / 1000;
    $rightentryprice = round(($texbox3 - $entval) * 1000) / 1000;
    $rightstoporderprice = round(($texbox2 + $entval) * 1000) / 1000;
    $rightvalidprice = $invalidprice;
    $rightretest = round(($texbox3 - $tval3) * 1000) / 1000;
    $rightinvalid = round(($texbox2 + $texbox1) * 1000) / 1000;
    $rightatr1entryprice = round(($texbox3 - $atr1entryval1) * 1000) / 1000;
    $rightatr1stoporderprice = round(($texbox3 - $atr1entryval2) * 1000) / 1000;
    $rightart1existprice = round(($texbox3 - $atr1entryval3) * 1000) / 1000;

    $rightatr2entryprice = round(($texbox3 - $atr2entryval1) * 1000) / 1000;
    $rightatr2stoporderprice = round(($texbox3 - $atr2entryval2) * 1000) / 1000;
    $rightart2existprice = round(($texbox3 - $atr2entryval3) * 1000) / 1000;

    // $rightatr3entryprice = intval(($texbox3 - $atr3entryval1) * 1000) / 1000;
      if ($selectmain == "ATRActual") {
        $rightatr3entryprice = intval(($texbox3 - $atr3entryval1)*1000)/1000;
        
    } elseif ($selectmain == "ATRTicks") {
        $rightatr3entryprice = round(($texbox3 - $atr3entryval1)*1000)/1000;
    } else {
        $rightatr3entryprice=0;
    }
    $rightatr3stoporderprice = round(($texbox3 - $atr3entryval2) * 1000) / 1000;
    $rightart3existprice = round(($texbox3 - $atr3entryval3) * 1000) / 1000;
    ...
    if ($selectrisk == "riskpoint") {
        ...
        $riskpoints = ($entprice - $stopprice) /0.001;
    }
    ...
    if ($riskpoints != 0) { // Check for division by zero
        if ($selectrisk == "riskpoint") {
            $nqcontracts = ($riskval / $riskpoints) / 10000;
        } elseif ($selectrisk == "riskticks") {
            $nqcontracts = (($riskval / $riskpoints) / 10000) / 0.001;
        }
    }
    ...
    $result['mnqcontracts'] = $mnqcontracts;
        ...
        if ($riskpoints != 0) { // Check for division by zero
            if ($selectshort == "longtarget" && $selectrisk == "riskpoint") {
                ...
                $riskreward = (($shorttarget - $entprice) / 0.001) / $riskpoints;
            } elseif ($selectshort == "shorttarget" && $selectrisk == "riskticks") {
                $riskreward = (($rightentryprice - $shorttarget) / 0.001) / $riskpoints;
    ...
    if ($atr1selectrisk == "riskpoint") {
        ...
        if ($atr1riskpoints != 0) {
            $atr1escontracts = ($riskval2 / $atr1riskpoints) / 10000;
        ...
        $atr1riskpoints = ($atr1shortstopprice - $atr1shortentryprice) / 0.001;
        if ($atr1riskpoints != 0) {
            $atr1escontracts = (($riskval2 / $atr1riskpoints) / 10000 ) /0.001;
            $atr1riskreward = (($atr1shortentryprice - $atr1shortexitprice) / 0.001) / $atr1riskpoints;
        } 
    }
    ...
    if ($atr2selectrisk == "riskpoint") {
        ...
        if ($atr2riskpoints != 0) {
            $atr2escontracts = ($riskval2 / $atr2riskpoints) / 10000;
            $atr2riskreward = ($atr2shortentryprice - $atr2shortexitprice) / $atr2riskpoints;
        }
    } elseif ($atr2selectrisk == "riskticks") {
        $atr2riskpoints = ($atr2shortstopprice - $atr2shortentryprice) / 0.001;
        if ($atr2riskpoints != 0) {
            $atr2escontracts = (($riskval2 / $atr2riskpoints) / 10000 ) / 0.001;
            $atr2riskreward = (($atr2shortentryprice - $atr2shortexitprice) / 0.001) / $atr2riskpoints;
        }
    }
    ...
    if ($atr3selectrisk == "riskpoint") {
        ...
        if ($atr3riskpoints != 0) {
            $atr3escontracts = ($riskval2 / $atr3riskpoints) / 10000;
            $atr3riskreward = ($atr3shortentryprice - $atr3shortexitprice) / $atr3riskpoints;
        }
    } elseif ($atr3selectrisk == "riskticks") {
        $atr3riskpoints = ($atr3shortstopprice - $atr3shortentryprice) / 0.001;
        if ($atr3riskpoints != 0) {
            $atr3escontracts = (($riskval2 / $atr3riskpoints) / 10000 ) / 0.001;
            $atr3riskreward = (($atr3shortentryprice - $atr3shortexitprice) / 0.001) / $atr3riskpoints;
        }
    }
