...
function textBox2($data,$result1)
{
    ...
    $validaprice = round(($texbox2 + $texbox1) * 40) / 40;
...
function atr2SelectCal($data,$result2,$result6,$result9)
{
    ...
    $atr1riskreward = floatval(str_replace(',','',$result9["atr1riskreward"]));
    ...
    if ($atr2selectrisk == "riskpoint") {
        $atr2riskpoints = $atr2shortstopprice - $atr2shortentryprice;
        $atr2escontracts = (($atr1riskreward / $atr2riskpoints) / 400);
        $atr2riskreward = ($atr2shortentryprice - $atr2shortexitprice) / $atr2riskpoints;
    } elseif ($atr2selectrisk == "riskticks") {
        $atr2riskpoints = ($atr2shortstopprice - $atr2shortentryprice) / 0.025;
        $atr2escontracts = (($atr1riskreward / $atr2riskpoints) / 400 )/ 0.025;
...
$result10 = atr2SelectCal($inputData,$result2,$result6,$result9);
