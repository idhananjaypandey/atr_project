...
function textBox2($data,$result1)
{
    ...
    $entprice = round(($texbox2 + $entval) * 4) / 4;
    $validaprice = round(($texbox2 + $texbox1) * 4) / 4;


    $selectmain = $data["selectmain"];

    if ($selectmain == "ATRActual") {
        $retestprice = floor(($texbox2 + $retest));
        
    } elseif ($selectmain == "ATRTicks") {
        $retestprice = round(($texbox2 + $retest));
    } else {
        $retestprice = 0; 
    }
    ...
    // $retestprice = floor(($texbox2 + $retest));
    $atr1shortentryprice = round(($texbox2 + $atr1entryval1));
    $atr1shortstopprice = round(($texbox2 + $atr1entryval2));
    $atr1shortexitprice = floor(($texbox2 + $atr1entryval3));

    $atr2shortentryprice = round(($texbox2 + $atr2entryval1));
    $atr2shortstopprice = round(($texbox2 + $atr2entryval2));
    $atr2shortexitprice = floor(($texbox2 + $atr2entryval3));

    $atr3shortentryprice = round(($texbox2 + $atr3entryval1));
    $atr3shortstopprice = round(($texbox2 + $atr3entryval2));
    $atr3shortexitprice = floor(($texbox2 + $atr3entryval3));

    $result['entprice'] = number_format($entprice, 0);
    $result['validaprice'] = number_format($validaprice, 0);
    $result['retestprice'] = number_format($retestprice, 0);
    $result['atr1shortentryprice'] = number_format($atr1shortentryprice, 0);
    $result['atr1shortstopprice'] = number_format($atr1shortstopprice, 0);
    $result['atr1shortexitprice'] = number_format($atr1shortexitprice, 0);
    $result['atr2shortentryprice'] = number_format($atr2shortentryprice, 0);
    $result['atr2shortstopprice'] = number_format($atr2shortstopprice, 0);
    $result['atr2shortexitprice'] = number_format($atr2shortexitprice, 0);
    $result['atr3shortentryprice'] = number_format($atr3shortentryprice, 0);
    $result['atr3shortstopprice'] = number_format($atr3shortstopprice, 0);
    $result['atr3shortexitprice'] = number_format($atr3shortexitprice, 0);
...
function textBox3($data,$result1)
{
    ...
    $entprice = 0;
    if($texbox3 > 0) {
         $entprice = round(($texbox3 - $entval) * 4) / 4;
    } 
    $stopprice = $entprice;

    $invalidprice = round(($texbox3 - $texbox1) * 4) / 4;

    $rightentryprice = $entprice;
    $rightstoporderprice = round(($texbox2 + $entval) * 4) / 4;
    $rightvalidprice = $invalidprice;
    $rightretest = round(($texbox3 - $retest) * 4) / 4;
    
    $rightinvalid = round(($texbox2 + $texbox1) * 4) / 4;
    $rightatr1entryprice = round(($texbox3 - $atr1entryval1) * 4) / 4;
    $rightatr1stoporderprice = round(($texbox3 - $atr1entryval2) * 4) / 4;
    $rightart1existprice = round(($texbox3 - $atr1entryval3) * 4) / 4;

    $rightatr2entryprice = round(($texbox3 - $atr2entryval1) * 4) / 4;
    $rightatr2stoporderprice = round(($texbox3 - $atr2entryval2) * 4) / 4;
    $rightart2existprice = round(($texbox3 - $atr2entryval3) * 4) / 4;

    $rightatr3entryprice = round(($texbox3 - $atr3entryval1) * 4) / 4;
    $rightatr3stoporderprice = floor(($texbox3 - $atr3entryval2) * 4) / 4;
    $rightart3existprice = round(($texbox3 - $atr3entryval3) * 4) / 4;

    $result['stopprice'] = number_format($stopprice,0);
    $result['invalidprice'] = number_format($invalidprice,0);
    $result['rightentryprice'] = number_format($rightentryprice,0);
    $result['rightstoporderprice'] = number_format($rightstoporderprice,0);
    $result['rightvalidprice'] = number_format($rightvalidprice,0);
    $result['rightretest'] = number_format($rightretest,0);
    $result['rightinvalid'] = number_format($rightinvalid,0);
    $result['rightatr1entryprice'] = number_format($rightatr1entryprice,0);
    $result['rightatr1stoporderprice'] = number_format($rightatr1stoporderprice,0);
    $result['rightart1existprice'] = number_format($rightart1existprice,0);
    $result['rightatr2entryprice'] = number_format($rightatr2entryprice,0);
    $result['rightatr2stoporderprice'] = number_format($rightatr2stoporderprice,0);
    $result['rightart2existprice'] = number_format($rightart2existprice,0);
    $result['rightatr3entryprice'] = number_format($rightatr3entryprice,0);
    $result['rightatr3stoporderprice'] = number_format($rightatr3stoporderprice,0);
    $result['rightart3existprice'] = number_format($rightart3existprice,0);
...
function callRisk($data,$result2,$result3) {
    ...
    if ($selectrisk == "riskpoint") {
        ...
        $riskpoints = ($entprice - $stopprice) / 1;
    }
...
function Contracts($data,$result4,$result5) {
    ...
    if ($riskpoints != 0) { // Check for division by zero
        if ($selectrisk == "riskpoint") {
            $nqcontracts = ($riskval / $riskpoints) / 5;
        } elseif ($selectrisk == "riskticks") {
            $nqcontracts = (($riskval / $riskpoints) / 5) / 1;
        }
    }
...
function atr1SelectCal($data,$result2,$result6)
{
    ...
    if ($atr1selectrisk == "Risk in Points") {
        $atr1riskpoints = $atr1shortstopprice - $atr1shortentryprice;
        $atr1riskpointsval = $atr1riskpoints;
        if ($atr1riskpoints != 0) {
            $atr1escontracts = ($riskval2 / $atr1riskpoints) / 5;
            $atr1riskreward = ($atr1shortentryprice - $atr1shortexitprice) / $atr1riskpoints;
        } 
    } else {
        $atr1riskpoints = $atr1shortstopprice - $atr1shortentryprice;
        if ($atr1riskpoints != 0) {
            $atr1escontracts = (($riskval2 / $atr1riskpoints) / 5) / 1;
            $atr1riskreward = (($atr1shortentryprice - $atr1shortexitprice) / 1) / $atr1riskpoints;
        } 
    }
...
function atr2SelectCal($data,$result2,$result6)
{
    ...
    if ($atr2selectrisk == "Risk in Points") {
        $atr2riskpoints = $atr2shortstopprice - $atr2shortentryprice;
        if ($atr2riskpoints != 0) {
            $atr2escontracts = ($riskval2 / $atr2riskpoints) / 5;
            $atr2riskreward = ($atr2shortentryprice - $atr2shortexitprice) / $atr2riskpoints;
        }
    } else {
        $atr2riskpoints = $atr2shortstopprice - $atr2shortentryprice;
        if ($atr2riskpoints != 0) {
            $atr2escontracts = (($riskval2 / $atr2riskpoints) / 5) / 1;
            $atr2riskreward = (($atr2shortentryprice - $atr2shortexitprice) / 1) / $atr2riskpoints;
        }
    }
...
function atr3SelectCal($data,$result2,$result6)
{
    ...
    if ($atr3selectrisk == "Risk in Points") {
        $atr3riskpoints = $atr3shortstopprice - $atr3shortentryprice;
        if ($atr3riskpoints != 0) {
            $atr3escontracts = ($riskval2 / $atr3riskpoints) / 5;
            $atr3riskreward = ($atr3shortentryprice - $atr3shortexitprice) / $atr3riskpoints;
        }
    } else {
        $atr3riskpoints = $atr3shortstopprice - $atr3shortentryprice;
        if ($atr3riskpoints != 0) {
            $atr3escontracts = ($riskval2 / $atr3riskpoints) / 5 / 1;
            $atr3riskreward = (($atr3shortentryprice - $atr3shortexitprice) / 1) / $atr3riskpoints;
        }
    }
