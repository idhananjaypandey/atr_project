...
function textBox2($data,$result1)
{
    ...
    $atr1shortentryprice = round(($texbox2 + $atr1entryval1)*4)/4;
    $atr1shortstopprice = floor(($texbox2 + $atr1entryval2)*4)/4;
    $atr1shortexitprice = round(($texbox2 + $atr1entryval3)*4)/4;

    $atr2shortentryprice = round(($texbox2 + $atr2entryval1)*4)/4;
    $atr2shortstopprice = round(($texbox2 + $atr2entryval2)*4)/4;
    $atr2shortexitprice = round(($texbox2 + $atr2entryval3)*4)/4;

    $atr3shortentryprice = round(($texbox2 + $atr3entryval1)*4)/4;
    $atr3shortstopprice = round(($texbox2 + $atr3entryval2)*4)/4;
    $atr3shortexitprice = round(($texbox2 + $atr3entryval3)*4)/4;
...
function Contracts($data,$result4,$result5) {
    ...
    if ($riskpoints != 0) { // Check for division by zero
        if ($selectrisk == "riskpoint") {
            $nqcontracts = ($riskval / $riskpoints) / 50;
        } elseif ($selectrisk == "riskticks") {
            $nqcontracts = (($riskval / $riskpoints) / 50) / 1;
        }
    }
...
function atr1SelectCal($data,$result2,$result6)
{
    ...
    if ($atr1selectrisk == "Risk in Points") {
        ...
        if ($atr1riskpoints != 0) {
            $atr1escontracts = ($riskval2 / $atr1riskpoints) / 50;
            $atr1riskreward = ($atr1shortentryprice - $atr1shortexitprice) / $atr1riskpoints;
        } 
    } else {
        $atr1riskpoints = $atr1shortstopprice - $atr1shortentryprice;
        if ($atr1riskpoints != 0) {
            $atr1escontracts = (($riskval2 / $atr1riskpoints) / 50) / 1;
...
function atr2SelectCal($data,$result2,$result6)
{
    ...
    if ($atr2selectrisk == "Risk in Points") {
        ...
        if ($atr2riskpoints != 0) {
            $atr2escontracts = ($riskval2 / $atr2riskpoints) / 50;
            $atr2riskreward = ($atr2shortentryprice - $atr2shortexitprice) / $atr2riskpoints;
        }
    } else {
        $atr2riskpoints = $atr2shortstopprice - $atr2shortentryprice;
        if ($atr2riskpoints != 0) {
            $atr2escontracts = (($riskval2 / $atr2riskpoints) / 50) / 1;
...
function atr3SelectCal($data,$result2,$result6)
{
    ...
    if ($atr3selectrisk == "Risk in Points") {
        ...
        if ($atr3riskpoints != 0) {
            $atr3escontracts = ($riskval2 / $atr3riskpoints) / 50;
            $atr3riskreward = ($atr3shortentryprice - $atr3shortexitprice) / $atr3riskpoints;
        }
    } else {
        $atr3riskpoints = $atr3shortstopprice - $atr3shortentryprice;
        if ($atr3riskpoints != 0) {
            $atr3escontracts = (($riskval2 / $atr3riskpoints) / 50) / 1;
